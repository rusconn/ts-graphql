...
import type { DB, Todo } from "../../../shared/mod.ts";
...
export class TodoQuery implements ITodoQueryForAuthed {
  ...
  constructor(db: ReadonlyKysely<DB>, repo: TodoRepo, tenantId?: Entity["userId"]) {
    this.#loaders = {
  ...
  async find(id: Entity["id"]) {
    ...
    return todo && Dto.fromEntity(todo);
  }
...
export function toDto(todo: Todo): Dto {
  return {
    id: todo.id,
    title: todo.title,
